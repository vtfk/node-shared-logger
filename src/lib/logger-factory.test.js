...
describe('Error handling', () => {
  it('catches errors from the remoteLogger function', () => {
    const { logger, mergedFakeDeps } = createLogger({
      loggerOptions: {
        localLogger: jest.fn((message) => {}),
        remoteLogger: {
    ...
    expect(() => logger('info', ['array', 123])).not.toThrow()

    const remoteLogger = mergedFakeDeps.loggerOptions.remoteLogger
    expect(remoteLogger.log).toHaveBeenCalled()
  })

  it('catches errors from the remoteLogger function and logges it locally', () => {
    const { logger, mergedFakeDeps } = createLogger({
      loggerOptions: {
        localLogger: jest.fn((message) => {}),
        remoteLogger: {
    ...
    const remoteLogger = mergedFakeDeps.loggerOptions.remoteLogger
    const localLogger = mergedFakeDeps.loggerOptions.localLogger.mock.calls
    expect(remoteLogger.log).toHaveBeenCalled()

    // Check if the error got logged locally
    expect(localLogger.length).toBe(2)
    expect(localLogger[0][0]).toContain('Test Error - R4nd0mC0d3')
    expect(localLogger[1][0]).toContain('array - 123')
  })
})